...
ADDON_NAME = "Telegram Stremio Media Server"
...
async def get_manifest():
    return {
        "id": "telegram.stremio.media.server",
...
async def get_catalog(media_type: str, id: str, extra: Optional[str] = None):
    ...
    if extra:
        if extra.startswith("search="):
            search_query = unquote(extra.removeprefix("search="))
        elif extra.startswith("skip="):
            try:
                stremio_skip = int(extra.removeprefix("skip="))
            except ValueError:
                stremio_skip = 0
    ...
    try:
        if search_query:
            ...
            if media_type == "movie":
                data = await db.sort_movies(sort_params, page=page, page_size=PAGE_SIZE)
                items = data.get("movies", [])
            else:
                data = await db.sort_tv_shows(sort_params, page=page, page_size=PAGE_SIZE)
